...
namespace MusicService.Controllers
{
    ...
    public class SongController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            ...
            {
                query = query.Where(s => s.Title.Contains(search) || s.Artist.Name.Contains(search));
            }
        ...
        [HttpPost("batch")]
        public async Task<IActionResult> GetSongsBatch([FromBody] List<int> ids)
        {
            if (ids == null || !ids.Any())
            ...
            var songs = await _context.Songs
                ...
                .Select(s => new
                {
                    ...
                    s.AudioUrl,
                    s.CoverUrl,
                    ArtistName = s.Artist.Name, // Flatten for easier frontend usage if needed, or keep object
                    Artist = new { s.Artist.Id, s.Artist.Name },
                    Album = s.Album == null ? null : new { s.Album.Id, s.Album.Title }
                })
                .ToListAsync();

            // Order by the order of IDs if possible? Not strictly necessary for this use case but nice.
